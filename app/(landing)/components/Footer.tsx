...
export default function Footer() {
	...
	const { profile, fetchProfile } = useSiteOwnerProfileStore();
	useEffect(() => {
		fetchProfile();
		console.log(3);
	});
	...
	return (
		<motion.footer
			className={clsx("border-t border-gray-200 py-8", {
		...
		>
			<div className="container mx-auto px-4">
				<div className="grid grid-cols-1 md:grid-cols-3 gap-8">
					...
					>
						<h2 className="font-bold text-lg mb-2">{profile?.brand}</h2>
						<p>{profile?.name}</p>
						<p>
							사업자등록번호:{" "}
							{formatBusinessRegistrationNumber(
